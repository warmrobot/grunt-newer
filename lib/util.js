...
var filterFilesByTime = exports.filterFilesByTime = function(files, previous,
    override, config, callback) {

  // You can define custom destination path, that Newer will check
  // customDest is a function that return custom path
  var customDest = config && config.options && config.options.newer;

  async.map(files, function(obj, done) {
    if (obj.dest) {
      fs.stat(customDest && customDest(obj.dest) || obj.dest, function(err, stats) {
        if (err) {
